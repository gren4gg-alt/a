...
| Alt | free the mouse, and take it back |
| G | fullscreen |
| Q | your character's ability |
| E | hold to pick a downed teammate up |
| V | push to talk |
| F2 | frame stats, seed and net role |
| Esc | also releases the mouse |

**Escape only goes one way.** Browsers release pointer lock on Escape themselves
and that cannot be reassigned, but there is no matching way back in, so getting
the cursor back meant clicking. **Alt** is a real toggle in both directions, and
**G** is fullscreen. Both are ordinary bindings in Settings → Controls and can
be changed like any other.

A key bound to something now has its browser default suppressed whether or not
the pointer is locked. Alt in particular focuses the browser menu bar otherwise,
so the game would lose the keyboard the moment you used it to free the cursor.
